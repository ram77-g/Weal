...
router.post('/', authenticateToken, async (req, res, next) => {
  try {
    ...
    res.status(201).json({ post: { ...post, author: post.isAnonymous ? null : post.author } });
  } catch (error) {
    next(error);
  }
});
...
router.put('/:id', authenticateToken, async (req, res, next) => {
  try {
    ...
    const post = await prisma.post.update({
      where: { id: postId },
      data: { title: title.trim(), content: content.trim(), imageUrl: imageUrl !== undefined ? imageUrl : existingPost.imageUrl },
      include: { author: { select: { id: true, name: true } } }
    });

    res.json({ post: { ...post, author: post.isAnonymous ? null : post.author } });
  } catch (error) {
    next(error);
  }
});
